...
@Component({
  selector: 'app-new-csr-category',
  templateUrl: './new-csr-category.component.html',
  styleUrls: ['./new-csr-category.component.scss']
})
export class NewCsrCategoryComponent {
bannerForm!: FormGroup;
  bannerUrl = '/api/csr/csrCategory';
  isEdit = false;
  responseData: any;
  bannerData: any;
  bannerUpdatedUrl = '';
    ...
    if (this.isEdit == false) {
      if (this.bannerForm.valid) {
        this.httpService.post(this.bannerUrl, this.bannerForm.getRawValue()).subscribe(response => {
          ...
          this.router.navigate(['/site/csr/csr-category']);
        },
