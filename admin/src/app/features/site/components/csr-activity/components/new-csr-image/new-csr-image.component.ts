...
@Component({
  selector: 'app-new-csr-image',
  templateUrl: './new-csr-image.component.html',
  styleUrls: ['./new-csr-image.component.scss']
})
export class NewCsrImageComponent {
  bannerForm!: FormGroup;
  bannerUrl = '/api/csr/csrDetail';
  categoryUrl = '/api/csr/csrCategory';
  categoryData:any;
  isEdit = false;
  responseData: any;
  bannerData: any;
  ...
  preFormData() {
    this.bannerForm.patchValue({
      ...
      csrCategoryUrlId: this.bannerData.csrCategoryUrlId,
  ...
  onSubmit() {
    if (this.isEdit == false) {
      if (this.bannerForm.valid) {
        ...
        formData.append('csrCategoryUrlId', this.bannerForm.get('csrCategoryUrlId')?.value);
        ...
        this.httpService.post(this.bannerUrl, formData).subscribe(response => {
          ...
          this.router.navigate(['/site/csr/csr-image']);
        },
