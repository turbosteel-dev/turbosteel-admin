...
@Component({
  ...
  styleUrls: ['./new-newsletter.component.scss']
})
export class NewNewsletterComponent {
  aboutUsListForm!: FormGroup;
  isEdit = false;
  aboutUsUrl = '/api/home/newsLetter'
  ...
  prePopulateForm() {
    ...
    const formattedDate = this.formatDate(this.companyListData.date);
    this.aboutUsListForm.patchValue({
  ...
  onSubmit() {
    if (this.isEdit == false) {
      if (this.aboutUsListForm.valid) {
        ...
        formData.append('date', this.aboutUsListForm.get('date')?.value);
        ...
        this.http.post(this.aboutUsUrl, formData).subscribe(response => {
          ...
          this.router.navigate(['/site/home/news-letter'])
        });
      }
    }
