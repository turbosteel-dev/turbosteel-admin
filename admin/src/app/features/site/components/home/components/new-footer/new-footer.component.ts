    ...
    if (this.isEdit == false) {
      if (this.bannerForm.valid) {
        ...
        this.httpService.post(this.bannerUrl, formData).subscribe(response => {
          this.snackBar.open('footer added successfully', '', { duration: 3000 });
          this.bannerForm.reset();
          this.router.navigate(['/site/home/footer']);
        },
          error => console.error('Error adding footer:', error)
        );
      }
    }
      ...
      this.httpService.put(url, formData).subscribe(response => {
        ...
        this.snackBar.open('footer updated successfully', '', { duration: 3000 });
        this.router.navigate(['/site/home/footer']);
      }, error => {
