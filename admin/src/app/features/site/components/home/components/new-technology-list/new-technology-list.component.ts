...
@Component({
  selector: 'app-new-technology-list',
  templateUrl: './new-technology-list.component.html',
  styleUrls: ['./new-technology-list.component.scss']
})
export class NewTechnologyListComponent {
  aboutUsListForm!: FormGroup;
  isEdit = false;
  aboutUsUrl = '/api/home/homeTechnologyList'
    ...
    if (this.isEdit == false) {
      if (this.aboutUsListForm.valid) {
        ...
        this.http.post(this.aboutUsUrl, formData).subscribe(response => {
          ...
          this.router.navigate(['/site/home/techology-list'])
        });
      }
    }
