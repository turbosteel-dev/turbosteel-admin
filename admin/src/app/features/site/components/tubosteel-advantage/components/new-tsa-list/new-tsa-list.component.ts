...
@Component({
  selector: 'app-new-tsa-list',
  templateUrl: './new-tsa-list.component.html',
  styleUrls: ['./new-tsa-list.component.scss']
})
export class NewTsaListComponent {
 aboutUsListForm!: FormGroup;
  isEdit = false;
  aboutUsUrl = '/api/turboSteel/turboSteelList';
    ...
    if (this.isEdit == false) {
      if (this.aboutUsListForm.valid) {
        ...
        this.http.post(this.aboutUsUrl, formData).subscribe(response => {
          ...
          this.router.navigate(['/site/turbosteel-advantage/turbosteel-list'])
        });
      }
    }
