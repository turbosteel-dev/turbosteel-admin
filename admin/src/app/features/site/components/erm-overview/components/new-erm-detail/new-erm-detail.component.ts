...
@Component({
  selector: 'app-new-erm-detail',
  templateUrl: './new-erm-detail.component.html',
  styleUrls: ['./new-erm-detail.component.scss']
})
export class NewErmDetailComponent {
aboutUsListForm!: FormGroup;
  isEdit = false;
  aboutUsUrl = '/api/erm/ermGroup'
    ...
    if (this.isEdit == false) {
      if (this.aboutUsListForm.valid) {
        ...
        this.http.post(this.aboutUsUrl, formData).subscribe(response => {
          ...
          this.router.navigate(['/site/erm/erm-detail'])
        });
      }
    }
