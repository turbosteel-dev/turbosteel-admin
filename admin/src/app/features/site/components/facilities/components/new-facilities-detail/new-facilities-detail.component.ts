...
@Component({
  selector: 'app-new-facilities-detail',
  templateUrl: './new-facilities-detail.component.html',
  styleUrls: ['./new-facilities-detail.component.scss']
})
export class NewFacilitiesDetailComponent {
 aboutUsListForm!: FormGroup;
  isEdit = false;
  aboutUsUrl = '/api/facilities/facilitiesDetail'
  ...
  categoryData: any;
  categoryUrl = '/api/facilities/facilitiesCategory'
  ...
  prePopulateForm() {
    this.aboutUsListForm.patchValue({
      ...
      facilities_url: this.companyListData.facilities_url,
  ...
  onSubmit() {
    if (this.isEdit == false) {
      if (this.aboutUsListForm.valid) {
        ...
        formData.append('facilities_url', this.aboutUsListForm.get('facilities_url')?.value);
        ...
        this.http.post(this.aboutUsUrl, formData).subscribe(response => {
          ...
          this.router.navigate(['/site/facilities/facilities-detail'])
        });
      }
    }
